...
//     // Wait for token to expire (1 hour + some buffer time)
//     await new Promise(resolve => setTimeout(resolve, 360000000 + 10000));
...
//       token = res.body.data.accessToken;
//       userId = res.body.data.user.userId; // Capture userId for later tests
//     });
...
//     // Attempt to access another user's organisation (which they shouldn't have access to)
//     const res = await request(app)
//       .get('/api/organisations')
//       .set('Authorization', `Bearer ${newToken}`);

//     expect(res.statusCode).toEqual(200);
//     expect(res.body.data.organisations).toHaveLength(0); // Ensure no unauthorized access
//   });
// });
